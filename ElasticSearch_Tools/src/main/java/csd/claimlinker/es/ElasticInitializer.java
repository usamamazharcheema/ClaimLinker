...
import csd.claimlinker.nlp.es.NLPlibEs;
...
public class ElasticInitializer {
    ...
    public void insertClaimsEval(boolean bulk) throws ClassNotFoundException, IOException {
    	this.nlp_instance = new NLPlibEs("data/stopwords.txt", "data/puncs.txt", "data/english-20200420.hash");
        ...
        ElasticInitializer.master_claim_record.forEach(elem -> {
            ...
            try {
                ...
                
                {
                    ...
                    String claimId = elem.get("num").toString();
                    String claimText = elem.get("vclaim").toString();
                    String claimExtraTitle = elem.get("title").toString();
                    String claimLabels = elem.get("vclaim_labels").toString();
                    
//                    Applying StopWords on claims text and title
                    CoreDocument claimTextDoc = this.nlp_instance.NLPlib_annotate(claimText);
                    CoreDocument claimExtraTitleDoc = this.nlp_instance.NLPlib_annotate(claimExtraTitle);
//                  
                    String claimTextStopWord = this.nlp_instance.getWithoutStopwords(claimTextDoc);
                    String claimExtraTitleStopWord = this.nlp_instance.getWithoutStopwords(claimExtraTitleDoc);
//                    
                    
                    //Applying wordNet expansion on claims text and title
                    CoreDocument claimTextWordDoc = this.nlp_instance.NLPlib_annotate(claimTextStopWord);
                    CoreDocument claimExtraTitleWordDoc = this.nlp_instance.NLPlib_annotate(claimExtraTitleStopWord);
//                    try {
                    String claimTextWordNet = this.nlp_instance.getLemmas_toString(claimTextWordDoc) + " " +claimTextStopWord;
                    String claimExtraTitleWordNet = this.nlp_instance.getLemmas_toString(claimExtraTitleWordDoc) +  " " +claimExtraTitleStopWord;
//                    
                	//this.nlp_instance.getWithoutStopwords(doc);

                    System.out.println("num = " + claimId);
                    System.out.println("vclaim = " + claimText);
                    System.out.println("title = " + claimExtraTitle);
                    System.out.println("claimLabels = " + claimLabels);
                    System.out.println("------");
                    
                    
                    
                   
                    builder.field("claimReview_url", claimId);
					builder.field("extra_title", claimExtraTitleWordNet);
					builder.field("claimReview_claimReviewed",claimTextWordNet);
					builder.field("claim_labels",claimLabels);
					builder.field("creativeWork_author_name", "");
	                builder.field("rating_alternateName", "");
	                builder.field("claim_uri", "");
...
//	                }
                ...
                builder.endObject();
                this.index(builder, bulk);
                System.out.printf("%.2f\r", (double) counter / 10000);
            } catch (IOException e) {
                e.printStackTrace();
            }
...
//            System.out.println("---------------");
	...
	public static void main(String[] args) throws ClassNotFoundException, IOException {
		...
		//clef2022/task2A/verified_claims.csv
		//clef2022/task2B/verified_claims.csv
		//verified_claims.csv   2020 verified claims
                        String data = "verified_claim_title.csv";
                        String ip = "localhost";
 
                        ElasticInitializer demo = new ElasticInitializer(data, ip, 9200, 9201);
                        demo.makeConnection();

                        demo.deleteClaims();
                        //demo.insertClaims(true);
                        demo.insertClaimsEval(true);
